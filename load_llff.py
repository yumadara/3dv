...
#TODO this should be used as additional check; since it changes and alters the order of the kezpoints
def refine_points_to_masked(basedir, factor=8, bd_factor=.75):
    data_file = Path(basedir) / 'colmap_depth.npy'
    ...
    # s_to = min(20000, len(points_keys))
    # p_is = np.random.randint(len(points_keys), size = s_to)
    # np_points = np.zeros((s_to, 3))
    # for i in range(s_to):
    #     p_i = p_is[i]
    #     np_points[i] = points[points_keys[p_i]].xyz
    #
    # fig = plt.figure()
    ...
    far = np.ndarray.max(bds_raw) * 1. * sc
    print('near/far:', near, far)
    intrinsics_m = np.loadtxt(basedir+"/intrinsics.txt")

    output_points = []

    for id_im in range(1, len(images) + 1):
        masked_im_fname = basedir + "/masks/" + images[id_im].name[:-4] + "alpi.png"
        m_im = np.asarray(Image.open(masked_im_fname)) / 255
        #plt.imshow(m_im[500:520, 600:620])
        #print(m_im[500:520, 600:620])
        #plt.show()
        for i in range(len(images[id_im].xys)):
            ...
            if id_3D == -1:
                continue
            point3D = points[id_3D].xyz
            pt = poses[id_im - 1, :3, 3]
            pr = poses[id_im - 1, :3, :3]
            #point3D_refCam = (pr @ point3D) + pt#It seems that is already in
            point3D_refCam = point3D  # It seems that is already in
            point2D_im = intrinsics_m[:3, :3] @ (point3D_refCam / point3D_refCam[2])
            point2D_im = np.round(point2D_im)
            px = int(point2D_im[0])
            py = int(point2D_im[1])
            # TODO check that is correct order
            # TODO first
            W, H = m_im.shape
            if px < 0 or px >= W or py < 0 or py >= H or m_im[px, py] == 0:
                continue
            else:
                print("Here!!!")
                output_points.append(points[id_3D])
    output_points_dict = {}
    for i, p in enumerate(output_points):
        output_points_dict[i] = p
    return output_points_dict
